...
public final class ComplianceEngine {
    ...
    public static Report check(Bitmap b, Requirement r){
        ...
        if(r.widthMm>0&&r.heightMm>0){
            float required=(float)r.widthMm/r.heightMm, actual=(float)b.getWidth()/b.getHeight();
            ...
            out.add(new Issue("Physical output","Exact millimetre output will be generated at export using the app's print conversion; the official source does not necessarily specify pixels.","WARNING"));
        } else out.add(new Issue("Size","Official source does not specify an exact measurement. Do not invent one; verify the form/institution requirement.","WARNING"));

        String bg=r.background==null?"":r.background.toLowerCase();
        if(bg.contains("tidak dinyatakan")||bg.contains("unspecified")||bg.trim().isEmpty()) out.add(new Issue("Background","Background is not specified by this official source. Verify the actual form/urusan.","WARNING"));
        else {
            out.add(new Issue("Background","Required: "+r.background+". The app cannot guarantee official background compliance from a simple colour sample; verify the final image.","WARNING"));
            if(bg.contains("putih")){int sample=0,white=0; int sy=Math.max(1,b.getHeight()/20),sx=Math.max(1,b.getWidth()/20); for(int y=0;y<b.getHeight();y+=sy)for(int x=0;x<b.getWidth();x+=sx){int p=b.getPixel(x,y);sample++;if(Color.red(p)>235&&Color.green(p)>235&&Color.blue(p)>235)white++;} if(sample>0&&white/(float)sample<0.35f)out.add(new Issue("Background sample","Large parts of the sampled image are not white. Check or replace the background.","WARNING"));}
        }
