...
public final class ProManager {
    private static final String PREF="passport_photo_pro"; private static final String KEY="active";
    private ProManager(){}
    ...
    public static boolean isPro(Context c){return c.getSharedPreferences(PREF,0).getBoolean(KEY,false);}
    public static boolean activate(Context c,String code){ if(code==null)return false; String expected=expectedCode(installationId(c)); if(code.trim().equalsIgnoreCase(expected)){c.getSharedPreferences(PREF,0).edit().putBoolean(KEY,true).putString("installation",installationId(c)).apply();return true;} return false; }
